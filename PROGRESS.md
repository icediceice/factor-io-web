...
- **How the template was edited:** hand-escaping `/` and `\"` into the 1MB single-line JSON string failed three times; the reliable route is a Node parse → plain-string replace → `JSON.stringify` pass, re-escaping `</` as `</` so a nested `</script>` cannot close the outer tag early. Script kept at `scratchpad/patch-template.js`. Each replacement asserts its anchor occurs exactly once, and the encode step proves the round-trip before writing.
- **Verified:** all four `__bundler/*` blocks still `JSON.parse`; every ld+json block on every page parses; all internal links resolve; sitemap locs all exist; homepage renders with the new nav item and light-tools card; section 04 shows the founder bio. Automated checker at `scratchpad/validate.js` — ALL CHECKS PASSED.
