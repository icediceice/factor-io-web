...
Contact is a LINE conversation, not a form. The contact page renders a QR figure plus a tappable `config.lineUrl` link (`https://line.me/ti/p/~<config.lineId>` — the two are asserted to agree, so the code and the image can never name different accounts), the LINE id as text, and the direct email address as a secondary channel inside its `<!--email_off-->` markers. The QR lives in `assetNames` so it is content-hashed like the stylesheet: a replaced QR must bust caches, because a stale one points at the wrong account. `contact()` receives the `asset()` helper threaded down from `renderPage` through `section()`, which is the only reason that parameter exists.
