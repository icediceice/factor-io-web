...
**Company profile added 2026-09-18 — a fifth route, `/profile/`.** Requested after a client asked for a company profile. It is written for a procurement or vendor-registration process rather than for a reader browsing the site, so it repeats material that already exists elsewhere (services, leadership, process, independence) in the compressed form those forms expect, and adds what no other page carried: the registered particulars. Seven sections — `company`, `registration`, `practice`, `leadership`, `engagement-model`, `delivery`, `standing` — all built from section kinds that already existed, so no template, CSS or new `kind` was introduced. Adding a route re-sequenced the page eyebrows: Contact moved from `03` to `04` in both locales.

The particulars are **compliance values, not copy**, and two of them are easy to break by tidying. The thirteen-digit `0105562205512` is simultaneously the company registration number and the tax identification number under Thai law, so one value answers both fields on a form. The registered Thai name is `บริษัท แฟคเคอร์ ไอ โอ จำกัด` — note the CONSONANT: `แฟคเคอร์`, not `แฟกเตอร์` or `แฟคเตอร์`. `quotes/lib/db.mjs` seed-settings calls this a Revenue Code s.86/4 mandatory particular of every tax invoice and says explicitly not to normalise it back; the About page had been carrying `แฟกเตอร์` and was corrected in the same change, because two different legal names on one site is precisely what a vendor registration rejects. The address romanisation (`Klong Sam Wa`, where RTGS would give `Khlong Sam Wa`) is likewise reproduced exactly as the operator supplied it. `tests/website.test.mjs` now pins every particular in both locales, pins the JSON-LD `taxID`/`telephone`/`PostalAddress`, and asserts that neither superseded Thai spelling appears in ANY generated output.
