...
import {
  ...
  saveRevision, markStatus, editRefusal, deleteQuotation, QuotationDeleteRefusal,
} from './lib/quote.mjs';
import {
...
export function createApi(db) {
  async function route(req) {
    try {
      ...
      if (e instanceof QuotationDeleteRefusal) return err(400, e.message);
  ...
  async function dispatch(req) {
    ...
    if (path === '/sow-templates') {
      if (method === 'GET') return json(200, { templates: parseTemplates(getSettings(db, '')['sow.templates']), starters: DEFAULT_SOW_TEMPLATES });
      if (method === 'PUT') {
    ...
    if (seg[0] === 'quotations' && seg[1]) {
      ...
      if (!seg[2]) {
        ...
        if (method === 'DELETE') {
          return json(200, deleteQuotation(db, id, { actor }));
        }
      }
