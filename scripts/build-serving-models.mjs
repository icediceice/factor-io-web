...
/**
 * Layer groups, in declaration order, collapsing consecutive same-kind runs.
 *
...
export function deriveParams(repoId, cfg, safetensorsTotal) {
  ...
  if (declared) {
    // A vendor-declared "-A3B" outranks any derivation: it is the model author's
    // own statement of the figure, and it is what the model is marketed as.
    const activeB = declared[1];
    return { params_b: billions(total, 2), active_params_b: activeB, active_basis: "declared", is_moe: geom.moe, fit };
  }
  if (!geom.moe) {
